#
# spec file for package SFEphp54-pear
#
%include Solaris.inc
...
%define _gpp g++
%include base.inc
%define php_major_version 5.4

%define usr_prefix /usr
%define peardir %{usr_prefix}/share/pear/%{php_major_version}
%define _prefix /usr/php/%{php_major_version}
%define _sysconfdir /etc/php/%{php_major_version}
%define xmlrpcver 1.5.4
%define getoptver 1.3.1
%define arctarver 1.3.11
%define structver 1.0.4
%define xmlutil   1.2.1
%define _pkg_docdir %{usr_prefix}/share/doc/SFEphp54-pear

Summary: PHP Extension and Application Repository framework
Name: SFEphp54-pear
IPS_package_name:        web/php-54/pear
...
Source1: %{name}_install-pear.php
Source2: %{name}_relocate.php
Source3: %{name}_strip.php
Source4: %{name}_LICENSE
Source5: %{name}_LICENSE-XML_RPC
Source10: %{name}_pear.sh
Source11: %{name}_pecl.sh
Source12: %{name}_peardev.sh
Source13: %{name}_macros.pear
...
/usr/php/%{php_major_version}/bin/php -n -dmemory_limit=32M -dshort_open_tag=0 -dsafe_mode=0 \
...
/usr/php/%{php_major_version}/bin/php -n %{SOURCE2} $RPM_BUILD_ROOT%{_sysconfdir}/pear.conf $RPM_BUILD_ROOT | 
  /usr/php/%{php_major_version}/bin/php -n %{SOURCE2} php://stdin $PWD > new-pear.conf
/usr/php/%{php_major_version}/bin/php -n %{SOURCE3} new-pear.conf ext_dir |
  /usr/php/%{php_major_version}/bin/php -n %{SOURCE3} php://stdin http_proxy > $RPM_BUILD_ROOT%{_sysconfdir}/pear.conf

/usr/php/%{php_major_version}/bin/php -r "print_r(unserialize(substr(file_get_contents('$RPM_BUILD_ROOT%{_sysconfdir}/pear.conf'),17)));"
...
%files
%defattr(-,root,bin)
%dir %attr (0755, root, sys) %{usr_prefix}/share
%{peardir}
%{_bindir}/*
...
%changelog
* Sat May 31 2014 YAMAMOTO Takashi <yamachan@selfnavi.com> - 1.9.4
- bump to 5.4.x
